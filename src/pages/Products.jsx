      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-96">
            <h3 className="font-bold mb-4">{currentProduct.id ? `Edit ${activeTab}` : `Add ${activeTab}`}</h3>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setCurrentProduct({
                      ...currentProduct,
                      crop: updatedCrop,
                      unit: autoPopulateUnit(updatedCrop, currentProduct.rateType),
                    });
                  }}
                >
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setCurrentProduct({
                      ...
                      unit: autoPopulateUnit(currentProduct.crop, updatedRateType),
                    });
                  }}
                >
...
>
  ...
  <option value="%v/v">%v/v</option>
                ...
                <input
                  ...
                  onChange={(e) => setCurrentProduct({ ...currentProduct, technology: e.target.value })}
                />
            ...
            {activeTab === 'Fertilizer' && (
              <>
                <input
                  ...
                  onChange={(e) => setCurrentProduct({ ...currentProduct, npk: e.target.value })}
                />
                <input
                  className="border px-2 py-1 mb-4 w-full"
            ...
            {activeTab === 'Chemical' && (
              <>
                <input
                  ...
                  onChange={(e) => setCurrentProduct({ ...currentProduct, chemicalType: e.target.value })}
                />
                <input
                  className="border px-2 py-1 mb-4 w-full"
                  placeholder="EPA Number"
                  value={currentProduct.epa || ''}
                  onChange={(e) => setCurrentProduct({ ...currentProduct, epa: e.target.value })}
                />
                <input
                  className="border px-2 py-1 mb-4 w-full"
                  placeholder="Active Ingredient (AI)"
                  value={currentProduct.ai || ''}
                  onChange={(e) => setCurrentProduct({ ...currentProduct, ai: e.target.value })}
                />
                <input
                  className="border px-2 py-1 mb-4 w-full"
                  placeholder="Carrier Volume Required (optional)"
                  value={currentProduct.carrierVolumeRequired || ''}
                  onChange={(e) => setCurrentProduct({ ...currentProduct, carrierVolumeRequired: e.target.value })}
                />
                <input
                  className="border px-2 py-1 mb-4 w-full"
                  placeholder="Manufacturer"
        ...
        <tbody>
          {filteredProducts.map(product => (
            <tr key={product.id}>
              ...
              {activeTab === 'Seed' && <td className="px-4 py-2">{product.crop}</td>}
              {activeTab === 'Seed' && <td className="px-4 py-2">{product.rateType}</td>}
              {activeTab === 'Chemical' && <td className="px-4 py-2">{product.chemicalType}</td>}
              {activeTab === 'Chemical' && <td className="px-4 py-2">{product.epa}</td>}
              {activeTab === 'Chemical' && <td className="px-4 py-2">{product.ai}</td>}
              {activeTab === 'Fertilizer' && <td className="px-4 py-2">{product.npk}</td>}
              <td className="px-4 py-2">{product.unit}</td>
              <td className="px-4 py-2">
                <button className="text-blue-600" onClick={() => { setModalOpen(true); setCurrentProduct(product); }}>Edit</button>
                <button className="text-red-600 ml-2" onClick={() => handleDeleteProduct(product.id)}>Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
