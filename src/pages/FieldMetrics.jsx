    ...
    <div className="p-4 md:p-6">
      ...
      <Card className="mb-4">
        ...
        <div className="text-base font-semibold text-blue-800">
          {metrics.totalFields} fields • {formatAcres(metrics.cropAcres)} crop
          ac • {formatAcres(metrics.totalAcres)} total ac
        </div>
        ...
        <div className="mt-3 text-xs text-gray-600">
          ...
          {cropTypes
            ...
            .map((crop) => {
              ...
              return (
                <div key={crop.name}>
                  {crop.icon} {crop.name} • {Math.round(acres)} ac •{" "}
                  {pct.toFixed(1)}%
                </div>
              );
            })}
      ...
      <Card className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <div className="text-sm text-gray-500">
            Raw FSA Acres by Crop (No Share Applied)
          </div>
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="text-gray-800">
            {(() => {
              ...
              const entries = Object.entries(cropTotals).map(
      ...
      <details open className="mb-4">
        <summary className="bg-white border rounded-lg px-4 py-2 font-semibold text-sm text-blue-800 cursor-pointer">
          Crop by County (Total FSA Acres)
        </summary>
        <Card className="border-t-0 rounded-t-none">
          <div className="text-sm text-gray-500 mb-2">
            Crop by County (Total FSA Acres, No Share Applied)
          </div>
          ...
          <div className="flex flex-wrap gap-3 items-center text-sm mb-3">
            ...
            <div>
              ...
              <input
                ...
                className="px-2 py-1 border rounded"
                onChange={(e) =>
                  setOperatorFilter(e.target.value.toLowerCase())
                }
              />
            </div>
            <div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-gray-600 border-b select-none">
              <tr>
                <th className="py-1 pr-4 cursor-pointer">County</th>
                <th className="py-1 pr-4 cursor-pointer">Operator</th>
                <th className="py-1 pr-4 cursor-pointer">Crop</th>
                <th className="py-1 text-right">Total Acres</th>
              </tr>
            </thead>
            <tbody className="text-gray-800">
              {(() => {
                ...
                fields.forEach((field) => {
                  ...
                  rows[key].acres += field.fsaAcres;
                });

                const filtered = Object.values(rows).filter((row) => {
                  return (
                    (!countyFilter ||
                      row.county.toLowerCase().includes(countyFilter)) &&
                    (!operatorFilter ||
                      row.operator.toLowerCase().includes(operatorFilter)) &&
                ...
                const total = filtered.reduce((sum, r) => sum + r.acres, 0);

                return (
                  <>
                    {filtered.map(({ county, operator, crop, acres }) => {
                      const cropType = cropTypes.find((c) => c.name === crop);
                      const icon = cropType?.icon || "❓";

                      return (
                        <tr
                          key={`${county}_${operator}_${crop}`}
                          className="border-b"
                        >
      ...
      <details open className="mb-4">
        ...
        <Card className="border-t-0 rounded-t-none">
          <div className="text-sm text-gray-500 mb-2">
            Crop by County (Operator Share × FSA Acres)
          </div>
                ...
                <th
                  ...
                  onClick={() =>
                    setCountySort({
                      column: "county",
                      direction:
                        countySort.direction === "asc" ? "desc" : "asc",
                    })
                  }
                >
                  County
            ...
            <tbody className="text-gray-800">
              {(() => {
                ...
                fields.forEach((field) => {
                  ...
                  const key = `${field.county}_${field.operator}_${crop}`;
                  const share =
                    field.fsaAcres * (field.operatorRentShare / 100);
                  if (!rows[key])
                    rows[key] = {
                      county: field.county,
                      operator: field.operator,
          ...
          <div className="flex flex-wrap gap-3 items-center text-sm mb-3">
            <div>
              ...
              <input
                ...
                className="px-2 py-1 border rounded"
                onChange={(e) =>
                  setOperatorNameFilter(e.target.value.toLowerCase())
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                className="px-2 py-1 border rounded"
                onChange={(e) =>
                  setFarmNumberFilter(e.target.value.toLowerCase())
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                className="px-2 py-1 border rounded"
                onChange={(e) =>
                  setOperatorCropFilter(e.target.value.toLowerCase())
                }
              />
            </div>
          </div>
                ...
                <th
                  ...
                  onClick={() =>
                    setOperatorSort({
                      column: "operator",
                      direction:
                        operatorSort.direction === "asc" ? "desc" : "asc",
                    })
                  }
                >
                  Operator
                ...
                let filtered = Object.values(rows).filter((row) => {
                  return (
                    (!operatorNameFilter ||
                      row.operator
                        .toLowerCase()
                        .includes(operatorNameFilter)) &&
                    (!farmNumberFilter ||
                      String(row.farmNumber)
                        .toLowerCase()
                        .includes(farmNumberFilter)) &&
                    (!operatorCropFilter ||
                      row.crop.toLowerCase().includes(operatorCropFilter))
                  );
                });
                ...
                return (
                  <>
                    {filtered.map(({ operator, farmNumber, crop, share }) => {
                      ...
                      return (
                        <tr
                          key={`${operator}_${farmNumber}_${crop}`}
                          className="border-b"
                        >
                          <td className="py-1 pr-4">{operator}</td>
          ...
          <div className="flex flex-wrap gap-3 items-center text-sm mb-3">
            <div>
              ...
              <input
                ...
                className="px-2 py-1 border rounded"
                onChange={(e) =>
                  setLandownerNameFilter(e.target.value.toLowerCase())
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                className="px-2 py-1 border rounded"
                onChange={(e) =>
                  setLandownerFarmFilter(e.target.value.toLowerCase())
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                className="px-2 py-1 border rounded"
                onChange={(e) =>
                  setLandownerCropFilter(e.target.value.toLowerCase())
                }
              />
            </div>
          </div>
                ...
                <th
                  ...
                  onClick={() =>
                    setLandownerSort({
                      column: "landowner",
                      direction:
                        landownerSort.direction === "asc" ? "desc" : "asc",
                    })
                  }
                >
                  Landowner
            ...
            <tbody className="text-gray-800">
              {(() => {
                ...
                fields.forEach((field) => {
                  ...
                  const share =
                    field.fsaAcres * (field.landownerRentShare / 100);
                  if (!rows[key]) {
                    rows[key] = {
                ...
                const filtered = Object.values(rows).filter((row) => {
                  return (
                    (!landownerNameFilter ||
                      row.landowner
                        .toLowerCase()
                        .includes(landownerNameFilter)) &&
                    (!landownerFarmFilter ||
                      row.farmNumber
                        .toLowerCase()
                        .includes(landownerFarmFilter)) &&
                    (!landownerCropFilter ||
                      row.crop.toLowerCase().includes(landownerCropFilter))
                  );
                });
                ...
                return (
                  <>
                    {filtered.map(({ landowner, farmNumber, crop, share }) => {
                      ...
                      return (
                        <tr
                          key={`${landowner}_${farmNumber}_${crop}`}
                          className="border-b"
                        >
