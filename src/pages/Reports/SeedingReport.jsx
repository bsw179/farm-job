...
Object.entries(vendorSummary)
  ...
  jobs.reduce((acc, job) => {
    ...
    const jobName = job.jobType?.name?.toLowerCase() || '';
    const cropName =
      fieldData.crops?.[cropYear]?.crop?.toLowerCase?.() ||
      fieldData.crop?.toLowerCase?.() ||
      "";
  ...
  return 0;
   ...
   return (
     <div className="p-6 space-y-8 max-w-7xl mx-auto">
       <h1 className="text-2xl font-bold">🌱 Seeding Report - {cropYear}</h1>{" "}
           ...
           return (
             <div key={operator} className="mt-4">
               <h3 className="font-bold">
                 {operator} – {totalAcres.toFixed(1)} acres
               </h3>
           ...
           <tbody>
             {Object.values(varietySummary).map((row) => (
               <tr key={`${row.variety}-${row.crop}`}>
                 ...
                 <td className="border px-2 py-1">{row.crop}</td>
                 <td className="border px-2 py-1 text-right">
                   {parseFloat(row.fieldAcres || 0).toFixed(1)}
                 </td>
                 <td className="border px-2 py-1 text-right">
                 ...
                 <td className="border px-2 py-1 text-right">
                   {parseFloat(row.totalAcres || 0).toFixed(1)}
                 </td>
                 <td className="border px-2 py-1 text-right">
                   {row.totalUnits.toFixed(1)} {row.unitLabel}
                 </td>
               </tr>
             ))}
           </tbody>
                   ...
                   <tr key={`${row.variety}-${row.crop}`}>
                     ...
                     <td className="border px-2 py-1">
                       {Object.entries(row.expenseSplit).map(([name, val]) => (
                         <div key={name}>
                           {name} - {val.toFixed(1)} {row.unitLabel}
                         </div>
                       ))}
                     </td>
                   </tr>
