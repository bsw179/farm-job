  ...
  (r) => r.productId === p.productId && r.unit === p.unit
   ...
   if (existing) {
     ...
     existing.vendors[vendorName] =
       (existing.vendors[vendorName] || 0) + amount;
   } else {
...
function getStandardUnit(product, appliedUnit) {
  ...
  if (type === "Chemical") return "gallons";
  if (unit.includes("oz") && unit.includes("dry")) return "lbs";
  return appliedUnit;
}
...
if (unit.includes("%") || unit.includes("v/v")) {
  ...
  if (unit.includes("oz") && unit.includes("dry")) {
    return appliedTotal / 16; // dry oz to lbs
  }
          ...
          <table className="min-w-full text-sm border">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="border px-2 py-1 text-right">Standardized</th>
                <th className="border px-2 py-1 text-left">Standard Unit</th>
                <th className="border px-2 py-1 text-left">Vendor Split</th>
              </tr>
            </thead>
            <tbody>
              {products.map((row) => {
                ...
                const standardUnit = getStandardUnit(product, row.unit);
                ...
                return (
                  <tr key={`${row.productName}-${row.unit}`}>
                    ...
                    <td className="border px-2 py-1">{standardUnit}</td>
