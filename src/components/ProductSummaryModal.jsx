...
productPurchases.forEach(p => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded shadow max-w-5xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">{product.name} ({product.type})</h2>
  ...
  <div className="border rounded p-3 bg-gray-50">
    ...
    <div>{totalNormalized.toLocaleString()} {productPurchases[0]?.unit || 'units'} ({parseFloat(productPurchases[0]?.amount || 0)} units)</div>
    {Object.entries(operatorPurchaseTotals).map(([name, val]) => (
  <div key={name} className="text-xs text-gray-600">
    {name}: {val.amount.toLocaleString(undefined, { maximumFractionDigits: 1 })} {productPurchases[0]?.unit}
    ({val.units.toFixed(1)} units)
  </div>
))}
</div>
...
<div className="border rounded p-3 bg-gray-50">
  ...
  <div>${totalCost.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
  {Object.entries(operatorPurchaseTotals).map(([name, val]) => (
    <div key={name} className="text-xs text-gray-600">
      {name}: ${val.cost.toLocaleString(undefined, { minimumFractionDigits: 2 })}
    </div>
  ))}
</div>
  ...
  <div className="border rounded p-3 bg-gray-50">
    ...
    <div>
      {(totalNormalized - Object.values(operatorApplied).reduce((a, b) => a + b, 0)).toLocaleString(undefined, { maximumFractionDigits: 1 })}
    ...
    {Object.entries(operatorApplied).map(([name, val]) => {
      ...
      return (
        <div key={name} className="text-xs text-gray-600">
          {name}: {remaining.toLocaleString(undefined, { maximumFractionDigits: 1 })} ({(remaining / parseFloat(productPurchases[0]?.unitSize || 1)).toFixed(1)} units)
        </div>
      );
    })}
  </div>
  ...
  <div className="border rounded p-3 bg-gray-50">
    ...
    <div>${avgUnitPrice.toFixed(2)}</div>
          ...
          <table className="w-full text-sm border mb-6">
            ...
            <tbody>
              {productPurchases.map(p => {
                const normalized = (parseFloat(p.amount) || 0) * (parseFloat(p.unitSize) || 1);
                const unitPrice = parseFloat(p.amount) > 0 ? (parseFloat(p.cost) || 0) / parseFloat(p.amount) : 0;
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td className="border px-2 py-1 text-right">{p.unitSize}</td>
                    <td className="border px-2 py-1 text-right">{normalized.toFixed(2)}</td>
                    <td className="border px-2 py-1 text-right">${parseFloat(p.cost).toFixed(2)}</td>
                    <td className="border px-2 py-1 text-right">${unitPrice.toFixed(2)}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div className="block md:hidden space-y-2 mb-6">
          {productPurchases.map(p => {
            const normalized = (parseFloat(p.amount) || 0) * (parseFloat(p.unitSize) || 1);
            const unitPrice = normalized > 0 ? (parseFloat(p.cost) || 0) / normalized : 0;
            return (
              <div key={p.id} className="border rounded p-3 shadow-sm bg-gray-50">
                <div className="text-xs uppercase text-gray-500 font-semibold">{p.unit}</div>
                <div className="font-semibold text-sm">{product.name}</div>
                <div className="text-sm text-gray-700">{p.amount} {p.unit} @ ${unitPrice.toFixed(2)} / {p.unit}</div>
                <div className="text-xs text-gray-500">{normalized.toLocaleString()} normalized • {p.date}</div>
              </div>
            );
          })}
        </div>
            ...
            {jobUsage.map((u, idx) => (
              <tr key={idx}>
                <td className="border px-2 py-1">
                  <Link to={`/jobs/${u.jobId}`} className="text-blue-600 underline">{u.date || '--'}</Link>
                </td>
                <td className="border px-2 py-1">{u.field}</td>
                <td className="border px-2 py-1 text-xs italic text-gray-600">{u.jobType}</td>
                <td className="border px-2 py-1">{u.operator}</td>
                <td className="border px-2 py-1 text-right">{u.rate}</td>
                <td className="border px-2 py-1 text-right">{u.acres}</td>
                <td className="border px-2 py-1 text-right">{u.applied.toFixed(2)}</td>
...
<div className="block md:hidden space-y-2 mb-6">
  {jobUsage.map((u, idx) => (
    <div key={idx} className="border rounded p-3 shadow-sm bg-gray-50">
      <div className="text-xs uppercase text-gray-500 font-semibold">{u.jobType}</div>
      <div className="font-semibold text-sm">{product.name}</div>
      <div className="text-sm text-gray-700">
        {u.applied.toLocaleString(undefined, { maximumFractionDigits: 1 })} {u.unit} applied
      </div>
      {isFinite(u.rate) && isFinite(avgUnitPrice) && (
  <div className="text-xs text-gray-500">
    Cost/acre: ${ (u.rate * avgUnitPrice).toFixed(2) }
      ...
      <div className="text-xs text-gray-500">{u.operator} • {u.crop}</div>
      <div className="text-xs text-gray-500">
        <Link to={`/jobs/${u.jobId}`} className="text-blue-600 underline">
          {u.date || '--'}
        </Link>
      </div>
    </div>
        ...
        <h3 className="text-lg font-semibold mb-2">Expense Split Summary Totals</h3>
<div className="text-sm">
  <div className="mb-2 font-semibold">Operators:</div>
  {Object.entries(operatorApplied).map(([name, val]) => {
    const cost = val * avgUnitPrice;
    const unitCount = val / parseFloat(productPurchases[0]?.unitSize || 1);
    return (
      <div key={name} className="ml-4">
        {name}: {val.toLocaleString(undefined, { maximumFractionDigits: 1 })} {jobUsage[0]?.unit}
        ({unitCount.toFixed(1)} units) • ${cost.toLocaleString(undefined, { minimumFractionDigits: 2 })}
      </div>
    );
  })}
